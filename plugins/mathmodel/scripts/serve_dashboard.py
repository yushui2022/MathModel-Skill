...
class DashboardHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _authorized(self) -> bool:
        ...
        supplied = self.headers.get("X-MathModel-Token") or urllib.parse.parse_qs(parsed.query).get("token", [""])[0]
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path.startswith("/api/jobs/"):
            try: self._send(json.dumps(self.runner.get(parsed.path.rsplit("/", 1)[-1]), ensure_ascii=False).encode("utf-8"), "application/json; charset=utf-8")
            except KeyError: self._send(b"Not found", "text/plain; charset=utf-8", 404)
            return
        if parsed.path.startswith("/api/artifact/") or parsed.path.startswith("/api/artifacts/"):
            try:
                artifact_id = parsed.path.split("/", 3)[-1]
                path, meta = self.state.read_artifact(artifact_id)
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if parsed.path != "/api/jobs":
            self._send(b"Not found", "text/plain; charset=utf-8", 404); return
        try:
            length = min(int(self.headers.get("Content-Length", "0")), 4096)
            payload = json.loads(self.rfile.read(length) or b"{}")
            job_id = self.runner.start(str(payload.get("type", "")))
            self._send(json.dumps({"job_id": job_id, "status": "running"}).encode("utf-8"), "application/json; charset=utf-8", 202)
        except Exception as exc:
            self._send(json.dumps({"error": str(exc)}, ensure_ascii=False).encode("utf-8"), "application/json; charset=utf-8", 400)
...
def main() -> int:
    ...
    parser.add_argument("--token", default="", help="Optional session token; a random token is generated by default.")
    ...
    DashboardHandler.runner = JobRunner(DashboardHandler.state)
    DashboardHandler.token = args.token if hasattr(args, "token") and args.token else secrets.token_urlsafe(24)
    with socketserver.ThreadingTCPServer((args.host, args.port), DashboardHandler) as server:
        actual_port = server.server_address[1]
        print(f"MathModel dashboard: http://{args.host}:{actual_port}/?token={DashboardHandler.token}")
