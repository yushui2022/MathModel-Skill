...
class DashboardHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/status":
            path = self.project_root / ".mathmodel" / "status.json"
            body = path.read_bytes() if path.exists() else json.dumps({
                "project": self.project_root.name,
                "stage": "S0",
                "stage_index": 0,
                "stage_count": 9,
                "status": "pending",
                "message": "No status has been recorded yet.",
                "artifacts": [],
                "history": [],
            }).encode("utf-8")
        ...
        if parsed.path.startswith("/project/"):
            relative = Path(urllib.parse.unquote(parsed.path.removeprefix("/project/")))
