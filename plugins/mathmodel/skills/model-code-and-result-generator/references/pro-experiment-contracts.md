...
Use schema 3.3. The complete field specification and run-spec example are in
...
For enabled `optimization_check_profile=linear-v1`, read
`quality-assurance-auditor/references/optimization-checks.md`. A supported run must
hash the approved specification and its data in `inputs`, and actually emit the
strict `optimization_result.json` as a recorded run output. Its objective must match
the preregistered scalar metric. Do not use a self-reported feasibility boolean or
solver status as independent validation. Keep Pro receipt generation unchanged.
