...
def main() -> int:
    ...
    try:
        from pro_optimization import enabled, validate_delivery
        if enabled(root):
            validators["optimization_specifications"] = lambda: validate_delivery(root)
    except (ValueError, OSError, KeyError, TypeError, AttributeError) as exc:
        errors.append(f"optimization profile cannot be read: {exc}")
