...
def main() -> int:
    ...
    parser.add_argument("--optimization-check-profile", choices=("linear-v1",),
                        help="Enable the explicit M3 companion; existing projects require fresh checkpoint decisions.")
    args = parser.parse_args()

    project_root = args.project_root.resolve()
    out = output_root(project_root, args.output_root)
    from pro_authoring_policy import make_policy
    previous_config = read_json(out / "pro_config.json") if (out / "pro_config.json").is_file() else {}
    previous = previous_config.get("paper_delivery", {})
    optimization_profile = args.optimization_check_profile or previous_config.get("optimization_check_profile")
    if optimization_profile not in (None, "linear-v1"):
        parser.error("unsupported existing optimization_check_profile; explicit migration is required")
    ...
    if optimization_profile is not None:
        config["optimization_check_profile"] = optimization_profile
