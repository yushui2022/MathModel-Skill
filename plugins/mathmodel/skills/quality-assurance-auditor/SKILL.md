...
- 启动或继续本 skill 的正式任务前，必须运行：
  ```bash
  python .agents/skills/paper-workflow-orchestrator/scripts/workflow_guard.py --skill quality-assurance-auditor
  ```
...
**在项目根目录运行**：
```bash
python .agents/skills/quality-assurance-auditor/scripts/pipeline.py
```

**正式成稿前运行证据门禁**：
```bash
python .agents/skills/quality-assurance-auditor/scripts/evidence_gate.py
```

**quickstart 验证时只看 warning**：
```bash
python .agents/skills/quality-assurance-auditor/scripts/evidence_gate.py --mode quickstart
```
