...
| 上下文恢复 | 11 项通过 | 纯读取、伪回执、来源分页、活动不扰动分页、畸形路径、跨项目记忆与失效；实际渲染工程项目的完成范围和交接在稿件修改后撤销 |
| 持久任务运行 | 12 项通过；Windows Python 3.11/3.12 分别执行 | 顺序队列、幂等与输入变化、独立重试、子进程树取消、日志容量、恢复；清理等待实际工作进程退出 |
...
已使用官方 Codex CLI `0.154.0` 从既有 `personal` marketplace 重装。发行 ZIP 为 `dist/MathModel-Codex-Plugin.zip`，版本 `0.5.0-alpha.1`；本机开发安装带官方 cachebuster 后缀 `+codex.20260910130729`。CLI 再读取确认 `mathmodel` 已启用，绝对命令指向安装缓存中的 MCP 脚本。最终发行 ZIP SHA-256：`2d425df0d74bd63209a1446884f70763694572eb36362c7e72eb5208011ddcfd`。
...
另用不继承旧对话的独立上下文读取安装入口、恢复包及来源，正确恢复已选/备用/淘汰路线、110/166/128.8 的计算结果，并指出同分候选之间缺少明确选模理由，未自行补造。该检查发现恢复摘要缺少直接的验收范围；现已将范围加入 JSON 与可复制摘要，完成交接不再产生重复写作任务。自动回归用真实插件指令哈希、计算和渲染验证上述状态在修改稿件后撤销。它仍只证明这个工程项目的恢复行为。
...
远程运行已通过 Ubuntu 两个 Python 版本的插件测试、Windows/Ubuntu 的固定基线、真实 LibreOffice 渲染及确定性发行包检查。Windows 插件检查先后检出 LICENSE 的 CRLF 漂移和任务终态后的目录清理竞态：已明确 LICENSE 使用 LF，并让测试按登记的进程创建标识等待真正退出，不忽略删除错误。修复后本机 Windows Python 3.11/3.12 的 12 项运行测试分别通过，完整远程矩阵状态以当前分支的 GitHub Actions 为准。
