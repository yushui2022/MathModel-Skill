...
本目录属于仓库保留的历史 Standard 样例，产物根为 `paper_output/`。下文的成果描述、数值、门禁报告和复核命令均对应这一历史版本，不能作为当前 Pro 插件 P0–P9 流程或论文质量验收的成果。原有赛题内容与产物保留供参考。

项目与版本说明见[主 README](../../README.md)，当前 Codex 插件的安装与使用见[插件 README](../../plugins/mathmodel/README.md)。当前插件可以恢复、展示实际新项目中已记录的题意、路线、运行与证据，Pro 新项目使用 `paper_output_pro/`；本样例的旧产物和批准不会自动迁移为 Pro 批准或验收结果。下文的平台包分发与新题操作说明适用于历史 Standard 路径，使用当前插件时应按插件 README 进入 Pro 流程。
