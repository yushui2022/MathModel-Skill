...
正式论文范式由 `paper-formal-writer` 负责：证据门禁通过后生成 `paper_outline.json`，按子问题数量给出动态篇幅目标，约束 Agent 写 `final_paper_source.md`，再生成含可编辑 Word OMML 公式的正式 Word，并检查 `1 / 1.1 / 1.1.1` 标题、图表引用、正文引文、重复内容和 required PDF 渲染。详见 [CUMCM Paper Standard](docs/cumcm-paper-standard.md) 和 [Formal Paper Authoring](docs/formal-paper-authoring.md)。
...
主分支继续以 Word 交付为默认主流程，避免给大多数用户增加额外安装和排版步骤。少数需要 LaTeX / PDF 交付的用户，可以切到 [`Latex` 分支](https://github.com/yushui2022/MathModel-Skill/tree/Latex) 使用可选 LaTeX 导出能力。
...
如果不是一次性跑完，而是隔了几轮对话、换了窗口或用户直接点名某个子 skill，Agent 应先回到 `paper-workflow-orchestrator`，运行对应平台的 `workflow_guard.py --status`，读取 `paper_output/qa/workflow_guard_report.json` 和 `paper_output/context/workflow_memory.json` 后再继续。
...
| [examples/cumcm2024-b-demo/paper_output/format_check_report.md](examples/cumcm2024-b-demo/paper_output/format_check_report.md) | 篇幅、标题层级、图表表格引用和 Word 结构检查。 |
...
继续一个中断过的项目时，不要让 Agent 凭聊天记录猜阶段。应要求它先运行 `workflow_guard.py --status`，然后按报告中的 `current_step`、`next_step`、`recommended_skill` 和 `next_action` 接着做。
...
| `paper_output/input_manifest.json` | `paper-workflow-orchestrator` | 数据清洗、建模、QA | 固定记录附件角色、字节数和 SHA-256，避免误分类或输入变化后继续使用旧产物 |
...
| `paper_output/data_cleaned/load_report.json` | `data-cleaning-and-visualization` | 建模、workflow guard | 记录数据读取状态与 `input_manifest_sha256`，防止旧清洗报告复用 |
| `paper_output/results/model_results.json` | `model-code-and-result-generator` | QA、微单元生成 | 保存每问模型输出、参数、方案、预测值或排序结果 |
| `paper_output/results/run_manifest.json` | `run_modeling.py` | workflow guard、evidence gate | 记录实际运行的脚本、命令、退出码、输入/输出大小和 SHA-256，防止手写伪造或事后篡改结果 |
...
| `paper_output/qa/evidence_gate_report.json` | `quality-assurance-auditor` | `paper-formal-writer`、workflow guard | 保存证据门禁状态和全部审计输入哈希 |
| `paper_output/plan/paper_outline.json` | `paper-formal-writer` | Agent、Word 排版、格式门禁 | 保存正式论文章节、目标字数、图表表格公式和证据引用要求 |
| `paper_output/format_check_report.json` | `paper-formal-writer` | workflow guard、最终 QA | 保存动态篇幅、原生公式、引文、渲染 QA 和输入哈希 |
...
- `quality-assurance-auditor`：作为证据门禁，重新核验建模脚本、输入和输出哈希，拒绝空或非有限指标、缺失/空/失败/占位图表表格以及过期运行记录；格式合格性由 `paper-formal-writer` 的 `check_paper_format.py` 负责。
- `paper-formal-writer`：证据门禁通过后生成正式 `paper_outline.json`，按子问题数量约束动态篇幅、`1 / 1.1 / 1.1.1` 标题、可编辑 Word OMML 公式、正文引文、Word 排版和 required render 格式门禁。
...
Claude 目录是 canonical payload；`sync_platform_packages.py` 用于检测 Codex 与 Trae 是否发生漂移。打包脚本会排除 `__pycache__/`、`*.pyc`、`problem_files/`、`crawled_data/`、`paper_output/` 和 `data_requirements.json`，并使用固定时间戳和权限生成可复现 zip。每个发布包都包含 `VERSION` 与 `MATHMODEL_BUILD.json`，清单记录版本、文件数、逐文件 SHA-256 和聚合 payload SHA-256；`--verify` 会确认现有归档与当前源码逐字节一致。
