...
def checkpoint_hashes(root: Path, checkpoint: str) -> dict[str, str]:
    ...
    if checkpoint == "1":
        ...
        # Explicit companion profile: existing schema-3.3 approvals are not upgraded.
        from pro_optimization import enabled, approval_hashes
        if enabled(root):
            result.update(approval_hashes(root))
    elif checkpoint == "3":
