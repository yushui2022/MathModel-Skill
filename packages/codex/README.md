...
把本目录下的 `skills/` 中的 skill 文件夹复制到你的 Codex skills 目录，或复制到项目根目录作为项目内 skills：
...
```text
AGENTS.md -> your-mathmodel-project/AGENTS.md
```
...
该命令只用于 quickstart / smoke test，输出是验证草稿，不代表正式比赛论文质量。正式赛题应由 Codex 先读取总控 skill，生成当前赛题专用代码、真实结果和证据门禁报告后，再调用 `paper-formal-writer` 生成正式 outline、全局写作 `final_paper_source.md`、排版 Word 并通过格式门禁。
