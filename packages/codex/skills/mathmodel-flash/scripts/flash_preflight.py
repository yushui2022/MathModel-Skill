...
def detect_mathmodel_installations() -> list[dict[str, str | None]]:
    ...
    for root_text in SKILL_ROOTS:
        ...
        for skill_dir in sorted(path for path in skills_root.iterdir() if path.is_dir()):
            ...
            if edition not in {"standard", "lite", "pro", "flash"}:
                continue
...
def main() -> int:
    ...
    configure_stdio()
    from flash_common import safe_path
    try:
    ...
    if len(editions) > 1:
        ...
        failures.append(f"当前入口是 Flash，但项目中检测到 {editions[0]} 安装。请移除其他版本后重试。")
    ...
    if len(versions) > 1:
        warnings.append(f"检测到多个 Flash 安装版本：{', '.join(versions)}。建议只保留当前版本。")
    if not files:
    ...
    manifest = {
        "schema_version": "1.0",
        "generated_by": "mathmodel-flash/scripts/flash_preflight.py",
    ...
    print(f"Flash input manifest: {rel(MANIFEST_FILE)}")
    if failures:
