...
- 必须存在：`paper_output/final_paper.docx`，但只有证据门禁通过后才能称为正式稿。
...
5. 证据门禁通过后，由 Agent 读取完整证据链全局撰写 `final_paper.md` 和 `final_paper.docx`。
6. 再次调用 `quality-assurance-auditor` 做最终一致性检查。
