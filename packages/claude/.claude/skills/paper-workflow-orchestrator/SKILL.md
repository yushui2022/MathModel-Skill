...
- 必须输出：`paper_output/OUTPUT_LAYOUT.md`、`problem_analysis.json`、`model_route.json`、`rubric_alignment.json`、`scoring_strategy.md`、数据/图表计划、`tasks.json`、微单元、`final_paper.md`、`final_paper.docx` 与 `ref_check.md`。
...
- 保持本项目的核心思路：以 skill 为主线，把“赛题解析 → 模型选择 → 数据处理 → 结果证据 → QA → 微单元生成 → 合并交付”串成一套可执行工作流。
- 注意：`scripts/run_all.py` 是可选稳定执行器，适合 quickstart、安装验证和已确认流程的重复执行；真实赛题中，Agent 仍应先读取本 workflow 与相关子 skill，再按当前赛题二次生成或修改数据处理、建模和图表代码，并把赛题专用代码写入 `paper_output/code/`。
...
- 当前赛题专用代码必须写入 `paper_output/code/`：数据处理放 `data_processing/`，绘图放 `visualization/`，建模放 `modeling/`，检查放 `qa/`。不要把 `q1_model.py`、绘图脚本或清洗脚本写回 `.claude/skills/*/scripts/`。
