...
```bash
python .claude/skills/pro-workflow-orchestrator/scripts/pro_preflight.py --platform claude-code --model "<用户声明模型>" --reasoning "<档位>" --multi-agent <available|unavailable|unknown> --network <available|unavailable|unknown> --parallel-tools <available|unavailable|unknown> --async-tools <available|unavailable|unknown>
```
