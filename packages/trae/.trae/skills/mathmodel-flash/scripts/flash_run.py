...
from flash_common import safe_path, configure_stdio
...
def input_failures(manifest: Any) -> list[str]:
    if not isinstance(manifest, dict) or manifest.get("status") != "PASS":
        return ["缺少通过的 input_manifest.json；请先运行 flash_preflight.py。"]
    failures: list[str] = []
...
def write_run(status: str, failures: list[str], completed: subprocess.CompletedProcess[str] | None, initial: dict | None = None) -> None:
    ...
    run = {
        "schema_version": "1.0",
        "generated_by": "mathmodel-flash/scripts/flash_run.py",
...
def main() -> int:
    configure_stdio()
    parser = argparse.ArgumentParser(description="Execute a Flash model with a configurable watchdog.")
    ...
    if not MODEL_FILE.exists() or MODEL_FILE.stat().st_size == 0:
        failures.append("缺少非空建模脚本：paper_output_flash/code/model.py")
    if failures:
    ...
    if not RESULTS_FILE.exists() or RESULTS_FILE.stat().st_size == 0:
        failures.append("model.py 未生成非空 paper_output_flash/results.json。")
    ...
    if failures:
        ...
        return 1
    print("[PASS] Flash model run recorded with input, script and output hashes.")
    return 0
